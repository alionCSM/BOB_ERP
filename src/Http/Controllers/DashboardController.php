...
final class DashboardController
{
    ...
    private function statsMacchine(): array
    {
        ...
        try {
            ...
            // migration non ancora applicata: meglio nessuna card che una
            // pagina che non si apre
            error_log('[Dashboard] macchine: ' . $e->getMessage());
            return [];
        }

        return [
            ['num' => max(0, $totali - $impegnate), 'label' => 'Macchine libere', 'sub' => 'oggi',
             'color' => '#16a34a', 'bg' => '#f0fdf4', 'href' => '/noleggi',
             'icon' => 'M3 21h18M6 21V8l12-5v18M10 12h4'],
            ['num' => $impegnate, 'label' => 'Macchine a noleggio', 'sub' => 'oggi',
    ...
    private function dataForDynamic(int $userId, string $name, \App\Domain\User $user): array
    {
        ...
        $catalog = [
            ...
            [['pn_noleggi'],               'Macchine',            'Piattaforme, carrelli, telescopici', '/noleggi',            '#7c3aed', '#f5f3ff', 'M3 21h18M6 21V8l12-5v18M10 12h4M10 16h4'],
        ];
